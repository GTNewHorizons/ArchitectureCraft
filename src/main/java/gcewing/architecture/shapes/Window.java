...
public abstract class Window extends ShapeKind {
    ...
    public boolean orientOnPlacement(EntityPlayer player, ShapeTE te, ShapeTE nte, EnumFacing otherFace, Vector3 hit) {
        ...
        if (!player.isSneaking() && (nte == null || !(nte.shape.kind instanceof gcewing.architecture.shapes.Window))) {
    ...
    public ShapeTE getConnectedWindowGlobal(ShapeTE te, EnumFacing globalDir) {
        ...
        if (thisFrameKind != FrameKind.None) {
            ...
            if (nte != null && nte.shape.kind instanceof gcewing.architecture.shapes.Window otherKind) {
